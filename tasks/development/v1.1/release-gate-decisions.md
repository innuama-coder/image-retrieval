...
TASK-007 acceptance-report 的 "VERDICT: BLOCKED / 10 gates OPEN" 结论以本决策为准更新。
...
| **MVP-001 / MVP-005** | 主观评价生产接入与 wire protocol | **Qwen 3.5 VLM 直连**（DashScope，凭据 `QWEN_API_TOKEN`）。门禁文档的 "OpenClaw" 为 v1.0 遗留表述，**v1.1 以 Qwen 3.5 VLM 为准**；不定义 OpenClaw 远程 wire protocol |
...
- 凭据 `QWEN_API_TOKEN` + endpoint `QWEN_API_BASE_URL`（DashScope）。
...
1. 真实服务冒烟（独立环节）：配 `SERPAPI_API_KEY` + `QWEN_API_TOKEN` + `QWEN_API_BASE_URL`，跑完整任务流程（SerpApi 搜索 + Qwen 评价 + web fetch 抓取 + 包校验）。
